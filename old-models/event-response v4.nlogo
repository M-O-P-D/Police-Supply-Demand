...
to setup
  ...
  ;let event-ref-file csv:from-file "input-data/crime-ref.csv"
  ...
  foreach event-ref-file [x -> show item 0 x]
  foreach event-ref-file [x -> show item 1 x]
  foreach event-ref-file [x -> show item 2 x]

  foreach event-ref-file [x -> table:put event-reference item 0 x (list item 1 x item 2 x)]
...
to manage-time
  ...
  if Hour > 23 [ set Day Day + 1 set Hour 0 ]
  if Hour >= 7 and Hour <= 14 [set Shift 1]
  if Hour >= 15 and Hour <= 21 [set Shift 2]
  if Hour < 7 or Hour >= 22 [set Shift 3]
...
to-report get-event-resource-time [ eventType ]
  ...
  ;in this 'stupid' case just apply a random poisson to that time to get the actual time to return
  let time round random-normal mean-time sd-time


 ;print (word eventType " - mean-time=" mean-time " ,sd-time=" sd-time " -- Actual=" time)
